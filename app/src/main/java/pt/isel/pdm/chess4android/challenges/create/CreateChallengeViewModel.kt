...
class CreateChallengeViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun createChallenge(name: String, message: String) {
        getApplication<DailyPuzzleApplication>().challengesRepository.publishChallenge(
            name = name,
            message = message,
            onComplete = {
                _created.value = it
    ...
    fun removeChallenge() {
        ...
        val repo = getApplication<DailyPuzzleApplication>().challengesRepository
    ...
    private fun waitForAcceptance(challengeInfo: ChallengeInfo) {
        subscription = getApplication<DailyPuzzleApplication>().challengesRepository.subscribeToChallengeAcceptance(
